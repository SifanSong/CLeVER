...
def eval_linear(args):
    ...
    if args.dataset_type == "Imagenet" or args.dataset_type == "IN100" or args.dataset_type == "CUB200":
        dataset_train = datasets.ImageFolder(os.path.join(args.data_path, "train"), transform=train_transform)

        if args.ds_mode.startswith("semi") and (args.semi_para in {1, 10}):
            if args.dataset_type == "Imagenet":
                ...
                if os.path.isdir('/home/sifan2/ImageNet/semi_files'):
                    args.train_files = open(f'/home/sifan2/ImageNet/semi_files/Imagenet_{args.semi_para}percent.txt').readlines()
                else:
                    args.train_files = open(f'/mnt/weka/wekafs/rad-megtron/ss3112/Datasets/ImageNet/semi_files/Imagenet_{args.semi_para}percent.txt').readlines()
            elif args.dataset_type == "IN100":
                if os.path.isdir('/home/sifan2/ImageNet/semi_files'):
                    args.train_files = open('/home/sifan2/ImageNet/semi_files/IN100_'+str(args.semi_para)+'percent.txt').readlines()
                else:
                    args.train_files = open('/mnt/weka/wekafs/rad-megtron/ss3112/Datasets/ImageNet/semi_files/IN100_'+str(args.semi_para)+'percent.txt').readlines()
            ...
            for fname in args.train_files:
                ...
                dataset_train.samples.append(
                    (args.data_path + "/train" +"/"+ cls +"/"+ fname, dataset_train.class_to_idx[cls]))
        ...
        if args.dataset_type == "Flowers102":
            if os.path.isdir('/home/sifan2/Datasets/'):
                dataset_train = datasets.Flowers102(root="/home/sifan2/Datasets/", split='train', download=True, transform=transform_train) # finished
                dataset_val = datasets.Flowers102(root="/home/sifan2/Datasets/", split='test', download=True, transform=transform_test) # finished
            else:
                dataset_train = datasets.Flowers102(root="/mnt/weka/wekafs/rad-megtron/ss3112/Datasets/Downstream/", split='train', download=True, transform=transform_train) # finished
                dataset_val = datasets.Flowers102(root="//mnt/weka/wekafs/rad-megtron/ss3112/Datasets/Downstream/", split='test', download=True, transform=transform_test) # finished
            print("Flowers102 has been loaded")
        elif args.dataset_type == "Food101":
            if os.path.isdir('/home/sifan2/Datasets/'):
                dataset_train = datasets.Food101(root="/home/sifan2/Datasets/", split='train', download=True, transform=transform_train) # finished
                dataset_val = datasets.Food101(root="/home/sifan2/Datasets/", split='test', download=True, transform=transform_test) # finished
            else:
                dataset_train = datasets.Food101(root="/mnt/weka/wekafs/rad-megtron/ss3112/Datasets/Downstream/", split='train', download=True, transform=transform_train) # finished
                dataset_val = datasets.Food101(root="/mnt/weka/wekafs/rad-megtron/ss3112/Datasets/Downstream/", split='test', download=True, transform=transform_test) # finished
            print("Food101 has been loaded")
        elif args.dataset_type == "OxfordIIITPet": ## 37 category
            if os.path.isdir('/home/sifan2/Datasets/'):
                dataset_train = datasets.OxfordIIITPet(root="/home/sifan2/Datasets/", split='trainval', download=True, transform=transform_train) # finished
                dataset_val = datasets.OxfordIIITPet(root="/home/sifan2/Datasets/", split='test', download=True, transform=transform_test) # finished
            else:
                dataset_train = datasets.OxfordIIITPet(root="/mnt/weka/wekafs/rad-megtron/ss3112/Datasets/Downstream/", split='trainval', download=True, transform=transform_train) # finished
                dataset_val = datasets.OxfordIIITPet(root="/mnt/weka/wekafs/rad-megtron/ss3112/Datasets/Downstream/", split='test', download=True, transform=transform_test) # finished
            print("OxfordIIITPet has been loaded")
        
        train_sampler = torch.utils.data.distributed.DistributedSampler(dataset_train)
        train_loader = torch.utils.data.DataLoader(
            dataset_train, batch_size=args.batch_size_per_gpu, shuffle=(train_sampler is None),
            num_workers=args.num_workers, pin_memory=True, sampler=train_sampler)

        val_loader = torch.utils.data.DataLoader(
            dataset_val, batch_size=256, shuffle=False,
    ...
    if args.evaluate: ## evaluate only for Imagenet and IN100
        if args.ds_mode.startswith("semi") or args.ds_mode == "ds_ft":
            utils.load_ft_model_weights(model, args.arch, args.patch_size, args.final_eval_weights)
    ...
    elif args.ds_mode.startswith("semi") or args.ds_mode == "ds_ft":
        pass
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print('model number of params (M): %.2f' % (n_parameters / 1.e6))
    parameters = list(filter(lambda p: p.requires_grad, model.parameters()))
    ...
    if args.ds_mode == "linear":
        ...
        init_linear_classifier_lr = args.lr * (args.batch_size_per_gpu * utils.get_world_size()) / 256.
        init_backbone_lr = args.backbone_lr * (args.batch_size_per_gpu * utils.get_world_size()) / 256.
        print("init_backbone_lr, init_linear_classifier_lr", init_backbone_lr, init_linear_classifier_lr)

        # classifier_parameters, model_parameters = [], []
        classifier_parameters = linear_classifier.parameters()
        model_parameters = model.parameters()
        param_groups = [dict(params=classifier_parameters, lr=init_linear_classifier_lr)]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if utils.is_main_process():
            ...
            # if args.else_part == "ALL":
            torch.save(save_dict, args.output_dir)
            # elif args.else_part == "main_part":
            #     torch.save(save_dict, args.output_dir)
            # elif args.else_part == "else_part":
            #     torch.save(save_dict, args.output_dir)
...
def train(model, linear_classifier, optimizer, loader, epoch, n, avgpool, args):
    ...
    for (inp, target) in metric_logger.log_every(loader, 20, header):
        ...
        if args.ds_mode == "linear":
            ...
            if "vit" in args.arch:
                intermediate_output = model.get_intermediate_layers(inp, n)
                output = torch.cat([x[:, 0] for x in intermediate_output], dim=-1) ## 每个取 cls_token ## concate last 4 cls tokens
                if avgpool:
                    output = torch.cat((output.unsqueeze(-1), torch.mean(intermediate_output[-1][:, 1:], dim=1).unsqueeze(-1)), dim=-1)
                    output = output.reshape(output.shape[0], -1)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_decay', default=0., type=float, metavar='W', help='weight decay (default: 0.)', dest='weight_decay')
